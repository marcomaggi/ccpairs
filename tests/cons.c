   ...
   reverse. */
{
  ...
  if (cce_location(L)) {
    ...
    P[4] = ccpairs_cons(L, 5, NULL); ccpairs_init_and_register_pair_clean_handler(L, &(P_H[4]), P[4]);
    P[3] = ccpairs_cons(L, 4, P[4]); ccpairs_init_and_register_pair_clean_handler(L, &(P_H[3]), P[3]);
    P[2] = ccpairs_cons(L, 3, P[3]); ccpairs_init_and_register_pair_clean_handler(L, &(P_H[2]), P[2]);
    P[1] = ccpairs_cons(L, 2, P[2]); ccpairs_init_and_register_pair_clean_handler(L, &(P_H[1]), P[1]);
    P[0] = ccpairs_cons(L, 1, P[1]); ccpairs_init_and_register_pair_clean_handler(L, &(P_H[0]), P[0]);
