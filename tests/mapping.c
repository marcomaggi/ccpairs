...
make_list_5 (cce_location_t * upper_L)
{
  ...
  if (cce_location(L)) {
    ...
    P[4] = ccpairs_cons(L, 5, NULL); ccpairs_init_pair_error_handler(L, &(P_H[4]), P[4]);
    P[3] = ccpairs_cons(L, 4, P[4]); ccpairs_init_pair_error_handler(L, &(P_H[3]), P[3]);
    P[2] = ccpairs_cons(L, 3, P[3]); ccpairs_init_pair_error_handler(L, &(P_H[2]), P[2]);
    P[1] = ccpairs_cons(L, 2, P[2]); ccpairs_init_pair_error_handler(L, &(P_H[1]), P[1]);
...
/* Single list mapping. */
{
  ...
  if (cce_location(L)) {
    ...
    ccpairs_init_list_clean_handler(L, P_H, P);
    {
      ...
      ccpairs_init_list_clean_handler(L, Q_H, Q);
